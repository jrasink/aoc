...
export const getInput = async ({ year, day, options }) => {
  const s = `0${day}`.slice(-2);
  const v = options.test ? 'test' : 'real';
  const path = `${year}/input/day-${s}/${v}`;
...
export const writeOutput = async ({ year, day, part, options }, { elapsed, result, logs }) => {
  ...
  const v = options.test ? 'test' : 'real';
  await fs.writeFile(`./${year}/output/day-${s}/part-${part}-${v}.json`, Buffer.from(JSON.stringify({ elapsed, result, logs }, null, 4)));
};
